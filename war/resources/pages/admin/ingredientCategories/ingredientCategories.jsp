	...
	<table id="ingredientCategoriesTable" class="table table-striped table-bordered">
	  ...
	  <tbody>
		...
		<tr height="50px" class="editableGrid" >
			<td>
				<a data-name="name" data-url="/admin/ingredientCategories?job=MODIFY" data-type="text" data-title="<fmt:message key="labels.name"/>" data-pk="${cat.id}" href="#"><c:out value="${cat.name}"/></a>
			</td>
			<td>
				<a data-name="description" data-url="/admin/ingredientCategories?job=MODIFY" data-title="<fmt:message key="labels.description"/>" data-type="textarea" data-pk="${cat.id}" href="#"><c:out value="${cat.description}"/></a>
			</td>
			<td>
				<a data-source="[{value: 'A', text: 'Aktywny'}, {value: 'I', text: 'Nieaktywny'}]" data-name="status" data-url="/admin/ingredientCategories?job=MODIFY" title="<fmt:message key="labels.status"/>" data-pk="${cat.id}" data-type="select" href="#" data-value="<c:out value="${cat.status}"/>"></a>
			</td>
			<td>
				<tags:remove-handler id="${cat.id}" params="id: ${cat.id}" url="ingredientCategory" label="buttons.action.remove" />
			</td>
		</tr>
		</c:forEach>
		<tr height="50px" class="editableFooter">
			...
			<td>
				<button class="btn btn-primary btn-sm" type="button" onclick="addItemToGrid();">
